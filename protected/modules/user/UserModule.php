...
class UserModule extends CWebModule
{
	...
	public $returnUrl = array("/user/profile/principal");
	...
	public static function esAdministrador() {
		if(Yii::app()->user->isGuest)
			...
			if( isset(Yii::app()->user->rol) && (Yii::app()->user->rol == 1 || Yii::app()->user->rol == 2) )
				return true;
			else
				return false;
		}
	...
	public static function esColaborador(){
		if(Yii::app()->user->isGuest)
			return false;
		else {
			if( isset(Yii::app()->user->rol) && Yii::app()->user->rol == 3 )
				return true;
			else
				return false;
		}
	...
	public static function esConsultor(){
		if(Yii::app()->user->isGuest)
			...
			if( isset(Yii::app()->user->rol) && Yii::app()->user->rol == 4 )
				return true;
			else
				return false;
		}
	...
	public static function esVisitante(){
		if(Yii::app()->user->isGuest)
			...
			if( isset(Yii::app()->user->rol) && Yii::app()->user->rol == 5 )
				return true;
			else
				return false;
		}
	...
	/**
	 * Todos los usuarios cuyo rol personalizado es Administrador (no super usuario)
	 */
	public static function getAdministradores() {
		if (!self::$_admins) {			
			$admins = User::model()->active()->findAll();
			$return_name = array();
			foreach ($admins as $admin){
				if( $admin->profile->rol == 1 || $admin->profile->rol == 2 )
					array_push($return_name,$admin->username);
			}
			self::$_admins = ($return_name)?$return_name:array('');
		}
