...
<ul class="navbar navbar-inverse  nav navbar-nav side-nav" id="menulateral">
	...
	<?php if( Yii::app()->getModule('user')->esAdministrador() ): ?>
		<li <?php if( strcmp(Yii::app()->controller->id,'comunicacion') == 0 && strcmp(Yii::app()->controller->action->id,'create') == 0 ): ?> class="active" <?php endif; ?>><?php echo CHtml::link('<i class="fa fa-fw fa-file-o"></i> Nueva Comunicación',array('comunicacion/create')); ?></li>
		<li <?php if( strcmp(Yii::app()->controller->id,'taller') == 0 ): ?> class="active" <?php endif; ?>><?php echo CHtml::link('<i class="fa fa-fw fa-text-height"></i> Talleres',array('taller/listar')); ?></li>
